...
OpenTag 会先判断被 @ 请求的交互类型：

- 问答、解释、能力咨询等只需回答的请求，在主会话中用一条普通文本直接回复。
- 改代码、跑测试、部署、排查等多步骤工作异步执行，并把状态、审批和最终结果留在话题中。
- 已经发生在话题里的回复始终留在原话题。
- `/chat` 可强制只回答，`/task` 可强制创建异步话题任务。
- 未 @ 的群聊消息默认保持安静；已经接受的异步任务完成后仍会主动回传结果，无需再次 @。

普通问答使用企业同事式表达：先说答案，简单问题控制在一到三句，不附带 run ID、
audit 命令或“还有什么可以帮助”的客服式结尾。任务话题仍保留必要的状态和审计能力，
但飞书中文界面会使用“我接了”“在处理了”“等你确认”等协作语言。若最近群聊上下文
因权限或授权失效无法读取，Tag 会明确说明缺少上下文，不会假装看过前面的讨论。
